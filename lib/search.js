...
function extractIds(html) {
  ...
  console.log('calling extract ids ' + count);
...
function idFetch(options, callback) {
  ...
  request(httpOptions, (err, response, body) => {
    ...
    console.log(' first page searchIds ' + searchIds.length);

    if (options.count > 25 && searchIds.length >= 25) {
      async.until(
        function() {
        ...
        function(callback) {
          ...
          const httpOptions = {url: query};
          console.log(Math.floor(searchIds.length / 25) + 1);
    ...
    [
      function(callback) {
        idFetch(options, (err, data) => {
          if (err) return callback(err);
          console.log('calling id Fetch ' + data);
